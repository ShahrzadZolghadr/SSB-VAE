%%writefile test_models_20news.py
...
def run_20_news(model_id,percentage_supervision,nbits_for_hashing,alpha_val,gamma_val,beta_VAL,name_file,addval=1,reseed=0,seed_to_reseed=20):
    ...
    #!gdown --id 1Ot7pEvJVrrB1hQCqUza9zR0WzhPH5d3l # download q_c concatenated vectors(50000)
    X = np.load('X.npy')
    y = pickle.load(open('y_50.pickle','rb'))
    Y = list(itertools.chain(*y))
    Y = np.array([0 if y==0 else 1 for y in Y]) # convert labels 2 to 1 in order to have 2 classes
